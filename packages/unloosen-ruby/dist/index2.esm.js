...
const main = async(rubyfile) => {
  const response = await fetch(buildExtensionURL("ruby-packed.wasm"));
  const buffer = await response.arrayBuffer();
  const module = await WebAssembly.compile(buffer);
  const { vm } = await new UnloosenVM().init(module);

  vm.printVersion();

  globalThis.rubyVM = vm;

  const script = await fetch(buildExtensionURL(rubyfile));
  vm.evalAsync(await script.text());
};

export { UnloosenVM, buildExtensionURL, main };

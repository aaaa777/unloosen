...
namespace :init do
    ...
    task :download_vfs do
        return unless Dir.glob("tools/#{vfsbin}").empty?
        sh "mkdir -p tools"
        vfsbin = "wasi-vfs-cli-x86_64-unknown-linux-gnu.zip"
        sh "cd tools && curl -LO https://github.com/kateinoigakukun/wasi-vfs/releases/download/v0.2.0/#{vfsbin} && unzip #{vfsbin}"
    end
